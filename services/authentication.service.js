...
const GOOGLE_CLIENT_ID = "944493246563-1l4t7vtcch01f5a8akvfb8o5locuqdtr.apps.googleusercontent.com"
    ...
    try {
        ...
        if (!savedUser) {
            let createUser = await MongoDB.db.collection(mongoConfig.collections.USERS)
                .insertOne({username:name, email, img:''})  
            const {_id} = createUser 
            const token = jwt.sign({_id,email},config.tokenSecret)
            ...
            let _id = (savedUser?._id).toString()
            const token = jwt.sign({_id,email},config.tokenSecret)
            return {
                message : 'success',
                token,
...
const postUpdate = async (post) => {
    ...
    let savedPost = await MongoDB.db.collection(mongoConfig.collections.POST).findOneAndUpdate({ _id: ObjectID(post?.id) }, { $set: { status: post?.status, amount: post?.amount, techie: post?.techie } },{returnOriginal: false,upsert:true})
    return savedPost
...
const tokenVerification = async (req, res, next) => {
    ...
    try {
        if (req?.originalUrl.endsWith('/') || req?.originalUrl.endsWith('/login') || req?.originalUrl.endsWith('/otp') || req?.originalUrl.endsWith('/register') || req?.originalUrl.endsWith('/userUpdate') || req?.originalUrl.endsWith('/refresh-token') || req?.originalUrl.endsWith('/add-admin') || req?.originalUrl.endsWith('/admin-login') || req?.originalUrl.endsWith('/lastseen') || req?.originalUrl.endsWith('/google-auth'))  {
            return next()
        }
...
module.exports = { userRegister, createOTP, userCheck, userLogin, userNameUpdate, tokenVerification, tokenRefresh, adminRegister, adminLogin, postUpdate,userLastSeenUpdate, userDpUpdate, googleLogin }